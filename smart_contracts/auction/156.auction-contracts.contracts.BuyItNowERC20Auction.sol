...
// File: contracts/AuctionComponents/BuyItNowAuction.sol

/**
 * @title BuyItNowAuction
 *
 * @author The AUX Team
 * @notice Contract for a single contract "Buy It Now" auction.
 */


contract BuyItNowAuction is AuctionBase, FeeCollector {
  using SafeMath for uint256;
  ...
  function bid(uint256 auctionId) whenNotPaused onlyAcceptingBids(auctionId) external payable {
    ...
    // Bidder must bid the correct amount
    uint256 price = auctionIdToPrice[auctionId];
    require(msg.value == price);

    auctionIdToAcceptingBids[auctionId] = false;

    transferWinnings(msg.sender, auctionId);
    auctionIdToSeller[auctionId].transfer(price);
  }

  function cancel(uint256 auctionId) whenNotPaused external onlySeller(auctionId) {
    auctionIdToAcceptingBids[auctionId] = false;
    transferWinnings(auctionIdToSeller[auctionId], auctionId);
  }

  /**
   * @notice Stores the requisite pricing information for a buy it now auction.
     Takes a 2% cut of the listing price
   */
  function setAuctionPricing(uint256 price, uint256 auctionId) requiresFee(price.div(50)) internal {
    auctionIdToPrice[auctionId] = price;
...
// File: contracts/Auctions/BuyItNowERC20Auction.sol

/**
 * @title BuyItNowCryptoKittyAuction
 *
 * @author The AUX Team
 * @notice Contract for a "Buy It Now" auction of a bundle of ERC20 tokens.
 */


contract BuyItNowERC20Auction is BuyItNowAuction, ERC20Auction {
  /**
  * @notice Creates and starts an auction with the given pricing and asset information.
  * @dev Composes the setup for the BuyItNowAuction, ERC20Auction  and AuctionBase.
  */
  function createAuction(uint256 price, address tokenAddress, uint256 tokenAmount) whenNotPaused public payable returns (uint256) {
    uint256 auctionId = createEmptyAuction();
    setAuctionPricing(price, auctionId);
