...
contract Auction {
    ...
    // Register new Bidder
    function register(address _account, uint8 _token) public valid (State.CREATED, true) { 
    ...
    // Start the session.
    function startSession() public valid (State.CREATED, true) {
        state = State.STARTED;
    }
    

    
    function bid(uint8 _price) public valid (State.STARTED, false){
    ...
    function announce() public valid (State.STARTED, true){ //only Auctioneer can annouced
    ...
    function getDeposit() public valid (State.CLOSING, false){ //only at closing state
