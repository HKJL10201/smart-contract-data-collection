    ...
    ExponentialNoError
{
    ...
    address internal bCompAddress;

    constructor(address newBCompAddress) {
        admin = msg.sender;
        bCompAddress = newBCompAddress;
    }
    ...
    function exitMarket(address cTokenAddress) external override returns (uint256) {
        ...
        (uint256 oErr, uint256 tokensHeld, uint256 amountOwed, ) = cToken.getAccountSnapshot(
            msg.sender
        );
    ...
    function _setBorrowPaused(CToken cToken, bool state) public override returns (bool) {
        require(markets[address(cToken)].isListed, "cannot pause a market that is not listed");
        require(
            msg.sender == pauseGuardian || msg.sender == admin,
            "only pause guardian and admin can pause"
    ...
    function _setSeizePaused(bool state) public override returns (bool) {
        require(
            msg.sender == pauseGuardian || msg.sender == admin,
            "only pause guardian and admin can pause"
        );
    ...
    function _setBCompAddress(address newBCompAddress) public override {
        // Check caller is admin
        require(msg.sender == admin, "unauthorized");

        // Set new bComp address
        bCompAddress = newBCompAddress;
    }
}
