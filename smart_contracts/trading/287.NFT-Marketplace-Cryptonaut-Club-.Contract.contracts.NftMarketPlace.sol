...
contract NftMarketPlace is ERC721URIStorage {
   ...
   // Offer for an NFT
    ...
    mapping(uint256 => Offer[]) private offersByTokenId;
    // Events
    event idMarketItemCreated(uint256 indexed tokenId, address seller, address buyer, uint256 price, bool sold);
    event OfferMade(uint256 indexed tokenId, address offerer, uint256 amount);
    // Modifiers
    modifier onlyOwner() {
    ...
    function makeOffer(uint256 _tokenId , uint256 price) public payable {
        require(idMarketItem[_tokenId].owner != address(0), "Token does not exist");
        require(idMarketItem[_tokenId].owner != msg.sender, "You are the owner of this token");
        require(price > 0, "Offer amount should be greater than 0");

        offersByTokenId[_tokenId].push(Offer(msg.sender, price));

        emit OfferMade(_tokenId, msg.sender, price);
    }
}
