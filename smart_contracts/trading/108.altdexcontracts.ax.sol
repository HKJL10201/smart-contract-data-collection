...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    uint256 public mintAllowanceStartTime; // Starting allowance time of minting of normal categories, MUST BE AT ICO, OR CLOSE TO IT. 
    ...
    constructor() {
        ...
        _emergencyMintAllowanceTime = (block.timestamp) + (2 * 52 * week); // Must be like 2 years after ico/mintAllowanceStartTime
        ...
        maxCategoryMint[MintingReason.EMERGENCY] = 20 * million * 10 ** _decimals; // 20 million emergency, contingent tokens
    ...
    function currentMintAllowance(MintingReason reason) public view returns (uint256) {
        ...
        if(reason == MintingReason.TEAM) {
            ...
            mintAllowanceTillNow = sqrt(80 * currentWeek()/52) * million * 10 ** _decimals; // 20 mil in 5 years
        } else if (reason == MintingReason.ECOSYSTEM) { // Make both the under ones so that they update weekly!
            // Using the function y = x^2.5. Creates an exponential supply distribution, non-linear. 
            
            mintAllowanceTillNow  = 2 * sqrt((currentWeek() ** 4) / (52 ** 4)) * million * 10 ** _decimals; // Ends at 20m in around 3 years.
        ...
        if(mintAllowanceTillNow > maxCategoryMint[reason]) {
            mintAllowanceTillNow = maxCategoryMint[reason]; // such that mintAllowance doesn't go over limit.
        }
