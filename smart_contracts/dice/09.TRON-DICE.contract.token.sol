...
contract BDC is ITRC20, Ownable, Mine {
    ...
    function getMinerInfo(uint id) public view onlyOwner returns(address, uint, bool, bool, bool, uint, uint, uint, uint, uint) {
        require(_minerID[id] != address(0), "invalid miner");
        MinerInfo storage info = _minerInfo[_minerID[id]];

        return (_minerID[id], info.id, info.ok, info.checkPrice, info.checkRate, info.minePrice, info.mineDevRate, info.amount, info.count, info.dividePay);
    }
    
    function dividePay() external payable onlyMiner {
        MinerInfo storage info = _minerInfo[msg.sender];
        if (info.id == 0) {
            return;
        }
        info.dividePay += msg.value;
        dividePoolAmount += msg.value;
    }
    ...
    uint constant public minUID = 0;
    uint public maxUID = minUID;
    ...
    function finduid(address addr) internal view returns (uint uid) {
        uid = _user2id[addr];
        require(uid != minUID, "invalid lock user");
        return uid;
    }
    /// uid
    ...
    mapping(uint => uint) private _lockBonus;
    ...
    mapping(uint => DivideTerm) public divideTermInfo;
    uint public divideTermID;
    
    function divideInfo() public view returns (uint, uint, uint) {
        DivideTerm storage info = divideTermInfo[divideTermID];
        
        return (totalLock, dividePoolAmount, info.divideRate);
    }
    
    function startDivide() public onlyDev {
        divideOngoing = true;
        
        DivideTerm storage info = divideTermInfo[divideTermID];
        info.lockedAmount = totalLock;
        info.divideAmount = dividePoolAmount * info.divideRate / 100;
        info.startTime = now;
        if (info.divideAmount == 0) {
            info.maxUID = maxUID;
            endDivide();
        }
        dividePoolAmount = dividePoolAmount - info.divideAmount;
    }
    
    function divide(uint cnt) public onlyDev returns (uint) {
        require(divideOngoing == true);
        
        DivideTerm storage info = divideTermInfo[divideTermID];
        
        uint start = info.maxUID;
        if (start == minUID) {
            start = minUID+1;
        }
        ...
        for (; start <= end; start++) {
            if (_lock[start] > 0) {
                divideBonus = info.divideAmount * _lock[start] / info.lockedAmount;
                _lockBonus[start] = _lockBonus[start].add(divideBonus);
            }
        }
        info.maxUID = end;
    ...
    function withdrawBonus(uint amount) public {
        uint uid = finduid(msg.sender);
        require(uid > minUID);
        if (amount == 0) {
            amount = _lockBonus[uid];
        }
        require(_lockBonus[uid] >= amount);
        
        msg.sender.transfer(amount);
        _lockBonus[uid] = _lockBonus[uid].sub(amount);
    }
    
    function bonus() public view returns (uint){
        uint uid = finduid(msg.sender);
        require(uid > minUID);
        return _lockBonus[uid];
    }
    
    function endDivide() public onlyDev {
        require(divideOngoing == true);
        DivideTerm storage info = divideTermInfo[divideTermID];
        require(info.maxUID == maxUID, "divide not end yet!");
        
        info.endTime = now;
        divideOngoing = false;
        divideTermID++;
        
        DivideTerm storage newInfo = divideTermInfo[divideTermID];
        newInfo.id = divideTermID;
        newInfo.divideRate = divideRate;
        newInfo.maxUID = minUID;
    }
