pragma solidity ^0.4.15; //We have to specify what version of the compiler this code will use
...
contract Voting {
    ...
    function totalVotesFor(bytes32 candidate) constant public returns (uint) {
    ...
    function indexOfCandidate(bytes32 candidate) constant public returns (uint) {
        for(uint i = 0; i < candidateList.length; i++) {
    ...
    function tokensSold() constant public returns (uint) {
        return totalTokens - balanceTokens;
    }

    function voterDetails(address user) constant public returns (uint, uint[]) {
    ...
    function allCandidates() constant public returns (bytes32[]) {
