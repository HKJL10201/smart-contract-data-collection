...
contract ProxyVoting {
	...
	modifier requireOpen {
		require(votingOpen(), "Voting period not open");
		_;
	}

	// require that voting has ended
	modifier requireEnded {
		require(now >= votingEnd, "Unavailable until voting period ends");
		_;
	}
	...
	// returns whether or not the voting period is open
	function votingOpen() public view returns (bool) {
		uint256 time = now;
		return time >= votingStart && time < votingEnd;
	}
