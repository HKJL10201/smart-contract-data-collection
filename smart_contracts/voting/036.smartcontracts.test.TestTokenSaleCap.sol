...
contract TestTokenSaleCap {
  ...
  function throwsWhenFinalizingWithIncorrectCap() {
    MultisigMock ms = new MultisigMock();
    XREDTokenSaleMock sale = new XREDTokenSaleMock(10, 20, address(ms), address(ms), 5, 1, 2);
    ms.deployAndSetXREDCoin(sale);
    ms.activateSale(sale);
  ...
  function testFundsAreTransferrableAfterSale() {
    MultisigMock ms = new MultisigMock();
    XREDTokenSaleMock sale = new XREDTokenSaleMock(1000000, 60000000, address(ms), address(ms), 3, 1, 2);
    ms.deployAndSetXREDCoin(sale);
    ms.activateSale(sale);
