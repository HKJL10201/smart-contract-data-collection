...
contract Bank2Contract {
    ...
    string  bank2to4_self ;
    string  bank2to5_self ;
    mapping(int => string) private PID;
    mapping(int => string) private Cert;
    mapping(string => string) private PIDList;
    mapping(string => string) private CertList;
    mapping(int => string) private VotingInfo;
    int counterPID=0;
    int counterCert=0;
    int counterVotingInfo=0;
  ...
  log private transactionLog;
    ...
    uint256 private bank_balance;    // total balance of Bank2
    bytes32 private bank_name = "Bank2";
    ...
    function Bank2Contract(uint256 initBalance, address fisc_account, address admin_account)
    {
  ...
  function getFiscAccount() constant returns (address) {
    ...
    if (daily_quota == 0) {  // use default setting
        ...
        uint256 res1 = bank_balance - amount; // 這行是扣款動作
        bank_balance = res1;
    ...
    function set_bank2to4(string a)  {
         bank2to4 = a ;
    }
    function set_bank2to5(string a) {
         bank2to5 = a ;
    }
    function get_bank2to4() constant returns(string) {
        return bank2to4;
    }
    function get_bank2to5() constant returns(string) {
        return bank2to5;
    }

    function set_bank2to4_self(string a)  {
         bank2to4_self = a ;
    }
    function set_bank2to5_self(string a) {
         bank2to5_self = a ;
    }
    function get_bank2to4_self() constant returns(string) {
        return bank2to4_self;
    }
    function get_bank2to5_self() constant returns(string) {
        return bank2to5_self;
    }
    ...
    function getNumOfVote()constant returns(int){
        return counterPID;
    }
    function setPID(string pid){

        PID[counterPID]=pid;
        PIDList[pid]=pid;
        counterPID++;
    }
    function getPID(int index)constant returns(string) {
        return PID[index];
    }
    function getPIDList(string index)constant returns(string) {
        return PIDList[index];
    }
    function setCert(string cert){
        Cert[counterCert]=cert;
        CertList[cert]=cert;
        counterCert++;
    }
    function getCert(int index)constant returns(string) {
        return Cert[index];
    }
    function getCertList(string index)constant returns(string) {
        return CertList[index];
    }
    function setVotingInfo(string ballotDetails){
        VotingInfo[counterVotingInfo]=ballotDetails;
        counterVotingInfo++;
    }
    function getVotingInfo(int index)constant returns(string) {
        return VotingInfo[index];
    }
}
