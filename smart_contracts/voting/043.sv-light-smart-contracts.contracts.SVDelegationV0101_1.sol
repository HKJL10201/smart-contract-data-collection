...
contract SVDelegationV0101_1 {
    ...
    function SVDelegationV0101_1(address prevDelegationSC) public {
        owner = msg.sender;
    ...
    function resolveDelegation(address voter, address tokenContract) public constant returns(uint64, uint64, uint64, address, address, address) {
        ...
        // CHANGE: Potential bug here: if (v1, t1) -> d1 on the old contract, and
        // there's a global v1 -> d2 on this smartcontract, then this function
        // resovleDelegation(v1, t1) -> d2 instead of d1. Oops. As it happens this
        // doesn't affect any delegations AFAIK, but is something to be aware of.
        // ------
    ...
    function findPossibleDelegatorsOf(address delegate) public view returns(address[] memory, address[] memory) {
        ...
        return resolveOlderDelegations(delegate, voters, tokenContracts);
    }

    // CHANGE: BROKE THIS OUT INTO IT'S OWN FUNCTION
    // this allows it to scan all known voters and tokens in the
    // past contract
    // ---
    // just get older delegations
    function resolveOlderDelegations(address delegate, address[] memory _voters, address[] memory _tcs) internal view returns (address[] memory voters, address[] memory tokenContracts) {
        ...
        address[] memory allPossibleSenders = new address[](43 + _voters.length);
        for (i = 0; i < allPossibleSenders.length; i++) {
            if (i < 43) {
                allPossibleSenders[i] = oldSenders[i];
            } else {
                allPossibleSenders[i] = _voters[i - 43];
            }
        }

        // loop through delegations in the previous contract
        for (i = 0; i < (allPossibleSenders.length); i++) {
            for (uint j = 0; j < logTokenContracts.length + 1; j++) {
                uint256 _oldId;
                address _oldDlgt;
                uint256 _oldSetAtBlock;
                uint256 _oldPrevId;
                address tc = j == 0 ? oldToken : logTokenContracts[j-1];
                if (tc == address(0)) {
                    (_oldId, _oldDlgt, _oldSetAtBlock, _oldPrevId) = prevSVDelegation._rawGetGlobalDelegation(allPossibleSenders[i]);
                } else {
                    (_oldId, _oldDlgt, _oldSetAtBlock, _oldPrevId) = prevSVDelegation._rawGetTokenDelegation(allPossibleSenders[i], tc);
                }

                if (_oldDlgt == delegate) {
                    voters = _appendMemArray(voters, allPossibleSenders[i]);
                    tokenContracts = _appendMemArray(tokenContracts, tc);
                }
            }
        }

        return (_concatMemArray(voters, _voters), _concatMemArray(tokenContracts, _tcs));
    }
    ...
    function _concatMemArray(address[] memory a1, address[] memory a2) internal pure returns (address[] memory a3) {
        a3 = new address[](a1.length + a2.length);
        for (uint i = 0; i < a3.length; i++) {
            if (i < a1.length) {
                a3[i] = a1[i];
            } else {
                a3[i] = a2[i - a1.length];
