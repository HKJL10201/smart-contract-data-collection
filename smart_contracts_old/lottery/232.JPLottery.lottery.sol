...
contract JackPotLottery is MyAdvancedToken {
    ...
    function JackPotLottery() MyAdvancedToken(countTickets*ticketPrice, "Simple Lottery Tocken", 0, "SLT") {
        clearTickets();
    }
    ...
    function play() onlyOwner {
        ...
        uint minNumberOfWin = lastWinNumber - (lastWinNumber%10);
        for (uint i = minNumberOfWin; i < minNumberOfWin * 10; i++) {
            balanceOf[tickets[i]] += ticketPrice * xPrize;
            clearTickets();
        }
    }

    function setLotteryParameters (uint newTicketPrice, uint newToJackPotFromEveryTicket, uint newXPrize) {
