...
contract Lottery {
    ...
    bool private mode = false; // false : answer for test, true : real block hash
    ...
    /*
     * @dev 베팅과 정댑 체크를 한다. 유저는 0.005 ETH를 보내야하고, 베팅용 1 byte 글자를 보낸다.
     * 큐에 저장된 배팅 정보는 이후에 distribute 함수에서 해결된다.
    ...
    /*
     * @dev 배팅을 한다. 유저는 0.005 ETH를 보내야 하고, 배팅용 1byte 글자를 보낸다.
     * 큐에 저장된 배팅 정보는 이후에 distribute 함수에서 해결한다.
     * @param challenges 유저가 배팅하는 글자.
     * @return 함수가 잘 수행되었는지 확인하는 bool 값.
     */
    ...
    /*
     * 베팅 결과값을 확인하고 팟머니를 분배한다.
     * 정답 실패 : 팟머니 축적, 정답 맞춤 : 팟머니 획득 or 정답 확인 불가 : 베팅 금액만 획득
     */
    function distribute() public {
        ...
        function transferAfterPayingFee(address payable addr, uint256 amount) internal returns (uint256) {
            ...
            /** 
             * eth 전송법 3가지 : call, send, transfer
             * call : 단순히 eth를 보내는 것 뿐만 아니라 특정 함수를 호출할 때도 사용 됨. 따라서 함수를 호출할 때 같이 eth를 보낼 수 있음.
             * send : 돈을 보냈더라도 boolean으로 false를 return한다. 그래서 트랜젝션이 fail이 나는 상황은 아님. 캐치를 해서 처리 해줌.
             * transfer : 가장 안전한 방법으로 많이 사용됨.
            */
